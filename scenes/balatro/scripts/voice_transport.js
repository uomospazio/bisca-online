...
(() => {
  class BiscaVoiceTransport {
    ...
    openPanel() {
      ...
      root.style.cssText='position:fixed;inset:0;z-index:2147483647;background:#241f1daa;display:grid;place-items:center;padding:12px;box-sizing:border-box';
      root.innerHTML=`<section role="dialog" aria-modal="true" aria-label="Chat vocale" style="box-sizing:border-box;width:min(480px,100%);max-height:90dvh;overflow:auto;background:#fdfdfb;color:#2a2438;border:3px solid #6f5fa8;border-radius:22px;padding:22px;font:600 16px system-ui"><h2 style="margin-top:0">CHAT VOCALE</h2><p>Premi ATTIVA AUDIO e consenti l'accesso nel browser. Anche i tuoi amici devono attivarla.</p><p data-status role="status"></p><div data-actions style="display:flex;gap:10px;flex-wrap:wrap"></div><div data-people></div><p style="font-size:13px">La partita continua mentre questo pannello e' aperto. Chiudere il pannello non spegne il microfono.</p></section>`;
      const actions=root.querySelector('[data-actions]');
      const button=(text,action)=>{const b=document.createElement('button');b.textContent=text;b.style.cssText='border:2px solid #fdfdfb;border-radius:14px;padding:12px;background:#6f5fa8;color:#241f1d;font:bold 15px system-ui;cursor:pointer';b.onclick=action;actions.appendChild(b);return b;};
    ...
    refreshPanel() {
      ...
      for(const [id,person] of this.people){
        ...
        const slider=document.createElement('input');slider.type='range';slider.min='0';slider.max='100';slider.value=String((this.volumes.get(id)??1)*100);slider.style.cssText='display:block;width:100%;accent-color:#6f5fa8';slider.setAttribute('aria-label',`Volume ${person.name || 'giocatore'}`);
