  ...
  window.BiscaProfile = {
    ...
    open(name) {
      ...
      root.innerHTML = `<style>
        .bisca-profile{background:#fdfdfb;color:#2a2438;border:3px solid #6f5fa8;border-radius:22px;box-shadow:5px 6px #241f1d;padding:22px;box-sizing:border-box;width:min(520px,94vw);max-height:94dvh;overflow:auto;text-align:center;font:700 17px system-ui;touch-action:auto}
        .bisca-profile [hidden]{display:none!important}.bisca-profile h2{margin:0 0 8px;font-size:25px}.bisca-profile p{margin:10px 0}.bisca-profile button{font:inherit;border:2px solid transparent;border-radius:14px;background:#6f5fa8;color:#fdfdfb;padding:12px 18px;cursor:pointer;box-shadow:3px 4px #241f1d;touch-action:manipulation}
        .bisca-profile button:hover{border-color:#fdfdfb}.bisca-profile button:disabled{opacity:.5;cursor:default}.bisca-profile .photo{display:block;margin:14px auto;width:128px;height:128px;border-radius:50%;border:3px solid #241f1d;background:#efecfa;padding:0;overflow:hidden;color:#2a2438;font-size:40px;box-shadow:none}.bisca-profile .photo img{width:100%;height:100%;object-fit:cover}.bisca-profile .choices{display:flex;gap:12px;justify-content:center;flex-wrap:wrap}.bisca-profile small{font-weight:400;display:block;margin:12px 0}.bisca-profile video{width:160px;height:160px;object-fit:cover;border-radius:50%}
      ...
      const useImage = source => {
        ...
        const ctx = canvas.getContext('2d'); ctx.fillStyle='#efecfa'; ctx.fillRect(0,0,192,192);
